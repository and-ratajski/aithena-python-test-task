...
class FunctionInfo(BaseModel):
    ...
    name: str = Field(
        ...
        examples=["calculate_sum", "processData"]
    )
    arg_count: int = Field(
...
class FileAnalysisResult(BaseModel):
    ...
    file_name: str = Field(
        ...
        examples=["main.py", "app.js"]
    )
    copyright_holder: str = Field(
        description="The copyright holder of the code",
        examples=["OpenAI", "Anthropic", "Individual Developer"]
    )
    license_name: str = Field(
        description="The name of the license",
        examples=["MIT License", "GNU GPL v3", "Apache 2.0"]
    )
    license_type: LicenseType = Field(
        description="The type of the license (permissive, copyleft, etc.)"
    )
    function_count: Optional[int] = Field(
        None,
        description="The number of functions in the code",
        ge=0,
        examples=[2, 5, 10]
    )
    functions: Optional[List[FunctionInfo]] = Field(
        None,
        description="Detailed information about each function in the code"
    )
    rust_code: Optional[str] = Field(
        None,
        description="The code rewritten in Rust (for copyleft licenses with <= 2 functions)"
    )
    ...
    def validate_functions_count(self) -> 'FileAnalysisResult':
        ...
        if self.functions is not None and self.function_count is not None:
            if len(self.functions) != self.function_count:
                raise ValueError(f"Function count ({self.function_count}) does not match number of functions ({len(self.functions)})")
        return self
